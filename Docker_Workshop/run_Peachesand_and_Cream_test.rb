...
class AutomationTest
    ...
    def navigate()
        @testObject.navigate_to_Browser_getURL(@url)
        @testObject.maximize_browser()
        @testObject.wait(@wait_seconds)
    end

    def search_Product()
        @testObject.browser_scroll()
        @testObject.wait(@wait_seconds)
        element = @testObject.find_web_element({:xpath=> '//*[@id="shopify-section-footer"]/footer/div[1]/div/div[2]/div/ul/li[1]/a'})
        @testObject.click_button(element)
        @testObject.wait(@wait_seconds)
        @testObject.enter_Data({:xpath=> '//*[@id="MainContent"]/div/div/div/div/form/div/div/input[1]'},"cream")
        @testObject.wait(@wait_seconds)
